<template>
    <div class="container-fluid" :style="{ backgroundColor: bgColor, height: '800px' }">
    ...
    <p class="text-center" :style="{ color: textColor }">현재 날짜: {{ currentDate }}</p>
    <p class="text-center" :style="{ color: textColor }">현재 시간: {{ currentTime }}</p>
    <input class="text-center" type="date" v-model="selectedDate">
    <input class="text-center" type="time" v-model="selectedTime">
    ...
    <div class="row justify-content-center">
        ...
        <div class="col-md-6">
            <!--화면의 중간부분을 의미-->
            <h1 class="text-center" :style="{ color: textColor }">BMI 계산기</h1>
            <!--글씨 가운데로-->
            <form @submit.prevent="calculateBMI">
            <!--submit는 새로고침, prevent는 막는다는 뜻. calculateBmi를 불러오는데 맘대로 새로고침은 되지 않도록함-->
        ...
        <div class="col-md-6 result" v-if="bmi !== null">
            ...
            <p>{{ bmi.toFixed(2) }}</p>
            <p>{{ BMICategory(bmi) }}</p>
            <img class="bmi--img" v-if="selectImg === '../assets/low.jpeg'" src='../assets/low.jpeg' style="max-width: 300px; max-height: 300%;">
            <img class="bmi--img" v-else-if="selectImg === '../assets/normal.jpeg'" src='../assets/normal.jpeg' style="max-width: 300px; max-height: 300px;">
            <img class="bmi--img" v-else-if="selectImg === '../assets/high.jpeg'" src='../assets/high.jpeg' style="max-width: 300px; max-height: 300px;">
            <img class="bmi--img" v-else-if="selectImg === '../assets/danger.jpeg'" src='../assets/danger.jpeg' style="max-width: 300px; max-height: 300px;">
